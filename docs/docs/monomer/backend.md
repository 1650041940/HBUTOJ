...
**项目依赖于hbutoj-redis，hbutoj-nacos，hbutoj-mysql等镜像成功启动，以及根据前面三个镜像的配置修改环境参数才可正常启动**
...
services:
  hbutoj-backend:
#    image: registry.cn-shenzhen.aliyuncs.com/hcode/hbutoj_backend
    ...
    environment:
      ...
      - NACOS_URL=172.20.0.4:8848 # hbutoj-nacos的url
      ...
      - MYSQL_HOST=172.20.0.3 # hbutoj-mysql的host
      - MYSQL_PUBLIC_HOST=172.20.0.3 # 如果判题服务是分布式，请提供当前mysql所在服务器的公网ip
      - MYSQL_PUBLIC_PORT=3306 # mysql主机端口号
      - MYSQL_PORT=3306 # mysql容器内端口号
      - MYSQL_DATABASE_NAME=hoj # 改动需要修改hbutoj-mysql镜像,默认为hoj
      - MYSQL_USERNAME=root 
      - MYSQL_ROOT_PASSWORD=hoj123456 # hbutoj-mysql的root账号密码
      ...
      - REDIS_HOST=172.20.0.2 # hbutoj-redis的host
      - REDIS_PORT=6379 # hbutoj-redis的port
      - REDIS_PASSWORD=hoj123456 #hbutoj-redis的密码
