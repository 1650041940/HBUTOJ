    ...
    <el-col :sm="24" :md="18" :lg="18">
      <el-card shadow>
          ...
          <section>
            ...
            <div>
              <el-tag
                ...
                class="filter-item"
                :effect="
                  selectedDifficultyRatingSegmentKey === 'All'
                    ? 'dark'
                    : 'plain'
                "
                @click="filterByDifficultyRatingSegment('All')"
                >{{ $t('m.All') }}</el-tag
              >
              <el-tag
                size="medium"
                class="filter-item"
                v-for="segment in difficultyRatingSegments"
                :effect="selectedDifficultyRatingSegmentKey === segment.key ? 'dark' : 'plain'"
                :key="segment.key"
                @click="filterByDifficultyRatingSegment(segment.key)"
                >{{ segment.label }}</el-tag
              >
            </div>
          </section>
        ...
        >
          ...
          >
            <template v-slot="{ row }">
              <span>{{ formatDifficultyRating(row.difficultyRating) }}</span>
            </template>
          </vxe-table-column>
    ...
    <el-col :sm="24" :md="6" :lg="6">
      ...
      <el-card :padding="10" style="margin-top:20px">
        <div slot="header" style="text-align: center;">
          <span class="taglist-title">{{ $t('m.Difficulty_Rating_Filter') }}</span>
        </div>
        <el-row :gutter="10">
          <el-col :span="12">
            <el-input-number
              v-model="query.minDifficultyRating"
              :min="800"
              :max="3500"
              :step="100"
              :step-strictly="true"
              ...
              style="width: 100%"
              :placeholder="$t('m.Difficulty_Rating_Min')"
              @change="filterByDifficultyRating"
            ></el-input-number>
          </el-col>
          <el-col :span="12">
            <el-input-number
              v-model="query.maxDifficultyRating"
              :min="800"
              :max="3500"
              :step="100"
              :step-strictly="true"
              ...
              controls-position="right"
              style="width: 100%"
              :placeholder="$t('m.Difficulty_Rating_Max')"
              @change="filterByDifficultyRating"
            ></el-input-number>
          </el-col>
        </el-row>
        <div style="margin-top: 10px; text-align: left; font-size: 12px; color: #909399;">
          <div>800-1100 {{ $t('m.Difficulty_Rating_Segment_Entry') }}</div>
          <div>1200-1500 {{ $t('m.Difficulty_Rating_Segment_Easy') }}</div>
          <div>1600-1900 {{ $t('m.Difficulty_Rating_Segment_Medium') }}</div>
          <div>2000-2300 {{ $t('m.Difficulty_Rating_Segment_Hard') }}</div>
          <div>2400+ {{ $t('m.Difficulty_Rating_Segment_Extreme') }}</div>
        </div>
        <div style="margin-top: 10px; text-align: center;">
          <el-button size="mini" type="primary" @click="applyDifficultyRatingRange">{{ $t('m.Filter') }}</el-button>
          <el-button size="mini" @click="resetDifficultyRatingRange">{{ $t('m.Reset') }}</el-button>
        </div>
      </el-card>
...
export default {
  ...
  data() {
    return {
      ...
      JUDGE_STATUS_RESERVE: {},
      REMOTE_OJ: {},
      difficultyRatingSegments: [
        {
          key: 'Entry',
          min: 800,
          max: 1100,
          label: '800-1100：' + this.$i18n.t('m.Difficulty_Rating_Segment_Entry'),
        },
        {
          key: 'Easy',
          min: 1200,
          max: 1500,
          label: '1200-1500：' + this.$i18n.t('m.Difficulty_Rating_Segment_Easy'),
        },
        {
          key: 'Medium',
          min: 1600,
          max: 1900,
          label: '1600-1900：' + this.$i18n.t('m.Difficulty_Rating_Segment_Medium'),
        },
        {
          key: 'Hard',
          min: 2000,
          max: 2300,
          label: '2000-2300：' + this.$i18n.t('m.Difficulty_Rating_Segment_Hard'),
        },
        {
          key: 'Extreme',
          min: 2400,
          max: 3500,
          label: '2400+：' + this.$i18n.t('m.Difficulty_Rating_Segment_Extreme'),
        },
      ],
  ...
  methods: {
    init() {
      ...
      const minDR = parseInt(query.minDifficultyRating);
      const maxDR = parseInt(query.maxDifficultyRating);
      this.query.minDifficultyRating = Number.isFinite(minDR) ? minDR : '';
      this.query.maxDifficultyRating = Number.isFinite(maxDR) ? maxDR : '';
      try {
    ...
    filterByDifficultyRatingSegment(key) {
      // 使用 difficultyRating 的 min/max 作为筛选条件；同时清空旧的 difficulty 等级筛选。
      this.query.difficulty = 'All';
      const segment = this.difficultyRatingSegments.find((s) => s.key === key);
      if (!segment) {
        this.query.minDifficultyRating = '';
        this.query.maxDifficultyRating = '';
      } else {
        this.query.minDifficultyRating = segment.min;
        this.query.maxDifficultyRating = segment.max;
      }
      this.query.currentPage = 1;
      this.pushRouter();
    },

    filterByDifficultyRating() {
      // 输入变化时不立即强制刷新，交由按钮或 change 触发；这里保持与其他筛选一致：回到第一页。
      this.query.currentPage = 1;
    },
    applyDifficultyRatingRange() {
      if (
        this.query.minDifficultyRating !== '' &&
        this.query.maxDifficultyRating !== '' &&
        this.query.minDifficultyRating > this.query.maxDifficultyRating
      ) {
        myMessage.warning(this.$i18n.t('m.Difficulty_Rating_Range_Invalid'));
        return;
      }
      this.query.currentPage = 1;
      this.pushRouter();
    },
    resetDifficultyRatingRange() {
      this.query.minDifficultyRating = '';
      this.query.maxDifficultyRating = '';
      this.query.currentPage = 1;
      this.pushRouter();
    ...
    getDifficultyRatingSegmentName(rating) {
      const value = rating == null || rating === 0 ? 800 : rating;
      if (value <= 1100) return this.$i18n.t('m.Difficulty_Rating_Segment_Entry');
      if (value <= 1500) return this.$i18n.t('m.Difficulty_Rating_Segment_Easy');
      if (value <= 1900) return this.$i18n.t('m.Difficulty_Rating_Segment_Medium');
      if (value <= 2300) return this.$i18n.t('m.Difficulty_Rating_Segment_Hard');
      return this.$i18n.t('m.Difficulty_Rating_Segment_Extreme');
    },
    formatDifficultyRating(rating) {
      const value = rating == null || rating === 0 ? 800 : rating;
      return value + ' (' + this.getDifficultyRatingSegmentName(value) + ')';
    },
    pickone() {
  ...
  computed: {
    ...
    selectedDifficultyRatingSegmentKey() {
      const min = this.query.minDifficultyRating;
      const max = this.query.maxDifficultyRating;
      if (min === '' && max === '') return 'All';
      const matched = this.difficultyRatingSegments.find(
        (s) => s.min === min && s.max === max
      );
