        ...
        >
          <template v-slot="{ row }">
            <el-input-number
              ...
              @change="(val, oldVal) => changeProblemDifficultyRating(row, val, oldVal)"
            ></el-input-number>
          </template>
        </vxe-table-column>
...
export default {
  ...
  methods: {
      ...
      if (this.routeName === 'admin-problem-list') {
        ...
        api.admin_getProblemList(params).then(
          (res) => {
            ...
            this.problemList.forEach((p) => {
              if (p.difficultyRating === null || p.difficultyRating === undefined) {
                p.difficultyRating = 1200;
              }
            });
    ...
    changeProblemDifficultyRating(row, val, oldVal) {
      if (val === oldVal) {
        return;
      }
      api
        .admin_changeProblemDifficultyRating({
          id: row.id,
