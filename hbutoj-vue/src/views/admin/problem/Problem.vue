          ...
          >
            ...
            >
              <el-select
                ...
                placeholder="Select difficulty rating segment"
                v-model="difficultyRatingSegmentKey"
                :disabled="problem.isRemote"
                @change="applyDifficultyRatingSegment"
              >
                <el-option
                  v-for="segment in difficultyRatingSegments"
                  :key="segment.key"
                  :label="formatDifficultyRatingSegmentLabel(segment)"
                  :value="segment.key"
                ></el-option>
              </el-select>
            </el-form-item>
          ...
          >
            ...
            >
              <el-input-number
                ...
                :disabled="problem.isRemote"
                @change="onDifficultyRatingChange"
              ></el-input-number>
            </el-form-item>
          </el-col>
        </el-row>
...
export default {
  ...
  data() {
    return {
      ...
      PROBLEM_LEVEL: {},
      JUDGE_CASE_MODE: {},
      difficultyRatingSegmentKey: 'Easy',
      difficultyRatingSegments: [
        {
          ...
          max: 1100,
          preset: 900,
          nameI18nKey: 'm.Difficulty_Rating_Segment_Entry',
        },
        {
          key: 'Easy',
          min: 1200,
          max: 1500,
          preset: 1300,
          nameI18nKey: 'm.Difficulty_Rating_Segment_Easy',
        },
        {
          key: 'Medium',
          min: 1600,
          max: 1900,
          preset: 1700,
          nameI18nKey: 'm.Difficulty_Rating_Segment_Medium',
        },
        {
          key: 'Hard',
          min: 2000,
          max: 2300,
          preset: 2100,
          nameI18nKey: 'm.Difficulty_Rating_Segment_Hard',
        },
        {
          key: 'Extreme',
          min: 2400,
          max: 3500,
          preset: 2400,
          nameI18nKey: 'm.Difficulty_Rating_Segment_Extreme',
        },
      ],
      spjRecord: {
        spjCode: "",
  ...
  methods: {
    init() {
      ...
      if (this.mode === "edit") {
        ...
        api[funcName](this.pid).then((problemRes) => {
          ...
          this.problem = data;
          this.difficultyRatingSegmentKey = this.getDifficultyRatingSegmentKeyByValue(
            this.problem.difficultyRating
          );
    ...
    formatDifficultyRatingSegmentLabel(segment) {
      if (!segment) return '';
      const rangeText = segment.key === 'Extreme' ? '2400+' : `${segment.min}-${segment.max}`;
      return `${rangeText}：${this.$i18n.t(segment.nameI18nKey)}`;
    },

    getDifficultyRatingSegmentKeyByValue(rating) {
      const value = rating == null || rating === 0 ? 800 : rating;
      if (value <= 1100) return 'Entry';
      if (value <= 1500) return 'Easy';
      if (value <= 1900) return 'Medium';
      if (value <= 2300) return 'Hard';
      return 'Extreme';
    },

    applyDifficultyRatingSegment(segmentKey) {
      const segment = this.difficultyRatingSegments.find((s) => s.key === segmentKey);
      if (!segment) {
        return;
      }
      this.problem.difficultyRating = segment.preset;
    },
    ...
    submit() {
      ...
      if (difficultyRating % 100 !== 0) {
        myMessage.error("难度分必须为整百数（如 800/900/.../3500）");
        return;
      }
