...
public class ProblemValidator {
    ...
    private void defaultValidate(Problem problem) throws StatusFailException {
        ...
        validateDifficultyRating(problem.getDifficultyRating());
    }

    public void validateDifficultyRating(Integer difficultyRating) throws StatusFailException {
        if (difficultyRating == null) {
            return;
        }
        if (difficultyRating < 800 || difficultyRating > 3500) {
            throw new StatusFailException("题目难度分范围请合理填写！(800~3500)");
        }
        if (difficultyRating % 100 != 0) {
            throw new StatusFailException("题目难度分必须为整百数！(如 800/900/.../3500)");
        }
    }
