...
public class AdminProblemServiceImpl implements AdminProblemService {
    ...
    @Override
    public CommonResult<Void> changeProblemDifficultyRating(Long pid, Integer difficultyRating) {
        try {
            adminProblemManager.changeProblemDifficultyRating(pid, difficultyRating);
            return CommonResult.successResponse();
        } catch (StatusFailException e) {
            return CommonResult.errorResponse(e.getMessage());
        } catch (StatusForbiddenException e) {
