...
public class AdminProblemManager {
    ...
    public void changeProblemDifficultyRating(Long pid, Integer difficultyRating) throws StatusFailException, StatusForbiddenException {

        problemValidator.validateDifficultyRating(difficultyRating);

        Problem problem = problemEntityService.getById(pid);
        if (problem == null) {
            throw new StatusFailException("修改失败！题目不存在！");
        }

        AccountProfile userRolesVo = (AccountProfile) SecurityUtils.getSubject().getPrincipal();
        boolean isRoot = SecurityUtils.getSubject().hasRole("root");
        boolean isProblemAdmin = SecurityUtils.getSubject().hasRole("problem_admin");
        if (!isRoot && !isProblemAdmin && (userRolesVo == null || !userRolesVo.getUsername().equals(problem.getAuthor()))) {
            throw new StatusForbiddenException("对不起，你无权限修改题目！");
        }
        ...
        boolean isOk = problemEntityService.update(updateWrapper);
        if (!isOk) {
            throw new StatusFailException("修改失败");
        }
        log.info("[{}],[{}],value:[{}],pid:[{}],operatorUid:[{}],operatorUsername:[{}]",
                "Admin_Problem", "Change_Difficulty_Rating", difficultyRating, pid, userRolesVo.getUid(), userRolesVo.getUsername());
    }
