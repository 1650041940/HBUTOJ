...
public class RejudgeManager {
    ...
    public Judge manualJudge(Long submitId, Integer status, Integer score) throws StatusFailException {
        ...
        if (Objects.equals(judge.getStatus(), Constants.Judge.STATUS_ACCEPTED.getStatus())
            ...
            QueryWrapper<UserAcproblem> existWrapper = new QueryWrapper<>();
            existWrapper.eq("uid", judge.getUid())
                .eq("pid", judge.getPid())
                .last("LIMIT 1");
            UserAcproblem existing = userAcproblemEntityService.getOne(existWrapper);
            if (existing == null) {
